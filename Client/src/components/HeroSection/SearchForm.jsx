          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`relative flex-1 flex items-center justify-center gap-2 px-6 py-4 text-sm font-semibold transition-all ${isActive ? "text-[#cf3425]" : "text-slate-500 hover:text-slate-800"
                  }`}
              >
                {isActive && (
                  <motion.div
                    layoutId="activeTab"
                    className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#cf3425]"
                  />
                )}
                <Icon size={16} />
                {tab.label}
...
const SearchButton = ({ text }) => (
  <div className="flex flex-col">
    <div className="h-[26px]" /> {/* spacer to align with labelled inputs */}
    <motion.button
