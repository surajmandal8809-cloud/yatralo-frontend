...
const LoginSuccessPage = () => {
  ...
  // With HashRouter, query params may be inside the hash segment
  // e.g.  /#/auth/login/success?token=xxx  →  window.location.search is empty
  // So extract from the hash part if needed
  const getTokenFromUrl = () => {
    ...
    // Fall back to parsing inside the hash  (#/path?token=...)
    const hash = window.location.hash; // e.g. "#/auth/login/success?token=abc"
  ...
  const userSession = getTokenFromUrl();
  ...
  useEffect(() => {
    if (userSession) {
      ...
      const timer = setTimeout(() => {
        // Use a full page reload so the Header remounts and reads
        // the fresh token from localStorage without any reactivity gap.
